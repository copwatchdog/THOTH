...
# === PAYROLL Enrichment (dynamic year, service_start tie-breaker) ===
    ...
    for row_idx, row in enumerate(rows, start=1):
        ...
        if service_start_dt:
            ...
            if asd:
                ...
                chosen_cells = (cells, delta_days)
            else:
                chosen_cells = (cells, None)
        else:
            chosen_cells = (cells, None)

        if chosen_cells:
            cells, delta = chosen_cells
            try:
                payroll_data = {
            ...
            break  # short-circuit after first qualifying match
    else:
        logging.warning(f"Payroll: no suitable payroll match found for '{query}' in years {target_years}")
