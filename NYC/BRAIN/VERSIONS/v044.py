...
# === NYPDTRIAL Extraction with retries ===
...
def _match_first_name(source_first: str, candidate_first: str) -> bool:
    if not source_first or not candidate_first:
        return False
    return _norm(source_first.split()[0]) == _norm(candidate_first.split()[0])
...
def _match_last_name(source_last: str, candidate_last: str) -> bool:
    ...
    # relaxed match: allow partial containment
    return (_norm(source_last_clean) in _norm(candidate_last_clean) or
...
def enrich_with_payroll(page, record):
    ...
    year_2024 = str(current_year - 1)
    year_2023 = str(current_year - 2)
    logging.info(f"Payroll: targeting {year_2024} first, then {year_2023}")
    ...
    for row_idx, row in enumerate(rows, start=1):
        ...
        # Only consider 2024 and 2023
        if year not in (year_2024, year_2023):
            continue

        # Strict normalized name check
        if _norm(first_name) != _norm(first) or _norm(last_name) != _norm(last):
            logging.info(f"Payroll: row#{row_idx} name mismatch (candidate '{first_name} {last_name}')")
            continue

        # === Priority 1: 2024, choose immediately and stop ===
        if year == year_2024:
            chosen = (cells, None)
            logging.info(f"Payroll: row#{row_idx} is {year}, chosen immediately, breaking loop")
            break

        # === Priority 2: 2023, tie-break by delta_days ===
        if year == year_2023:
            delta_days = None
            if service_start_dt:
                ...
                if asd:
                    ...
                    logging.info(f"Payroll: row#{row_idx} matched 2023; agency_start delta_days={delta_days}")

            if not chosen:
                chosen = (cells, delta_days)
                logging.info(f"Payroll: row#{row_idx} tentatively chosen (2023)")
            else:
                _, current_delta = chosen
                if delta_days is not None and (current_delta is None or delta_days < current_delta):
                    chosen = (cells, delta_days)
                    logging.info(
                        f"Payroll: row#{row_idx} replaces previous 2023 (smaller delta {delta_days} < {current_delta})"
                    )
