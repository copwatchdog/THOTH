    ...
    for sel in anchor_selectors:
        ...
        if el:
            ...
            if href:
                ...
                # PCT number logic remains the same
...
def enrich_with_payroll(page, record):
    ...
    for row_idx, row in enumerate(rows, start=1):
        ...
        logging.info(f"Payroll: row#{row_idx} -> year={year}, first='{first_name}', last='{last_name}', agency_start='{agency_start_date}'")
        ...
        if service_start_dt:
            ...
            if asd:
                ...
                logging.info(f"Payroll: row#{row_idx} matched name; agency_start delta_days={delta_days}")
                if not chosen:
                    chosen = (cells, delta_days)
                    logging.info(f"Payroll: row#{row_idx} tentatively chosen (delta {delta_days})")
                else:
                    _, current_delta = chosen
                    if current_delta is None or delta_days < current_delta:
                        chosen = (cells, delta_days)
                        logging.info(f"Payroll: row#{row_idx} replaces previous chosen (smaller delta {delta_days} < {current_delta})")
            else:
                logging.info(f"Payroll: row#{row_idx} matched name but agency_start_date unparsable; keeping as fallback if none chosen")
                if not chosen:
                    chosen = (cells, None)
        else:
            # No service_start to compare — pick the first exact name match deterministically
            chosen = (cells, None)
            logging.info(f"Payroll: row#{row_idx} chosen (first exact name match, no service_start present)")
            break
